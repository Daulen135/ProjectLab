    ...
    public static function generateField($key, $field)
    {
        ...
        if (is_array($field)) {
            ...
            if (isset($field['type'])) {
                ...
                $html .= '<label class="control-label col-sm-3" for="gateway-' . $key . '">' . \Yii::t('app', Inflector::titleize($key)) . '</label><div class="col-sm-6">';

                switch ($field['type']) {
                    case self::KEY_TYPE_BOOL:
                        $html .= "<input type='checkbox' " . $required . " value='" . $value . "' class='form-control' name='Value[" . $key . "]' placeholder='" . \Yii::t('app', Inflector::titleize($key)) . "'>";
                        break;
                    case self::KEY_TYPE_STRING:
                        $html .= "<input type='text' " . $required . " value='" . $value . "' class='form-control' name='Value[" . $key . "]' placeholder='" . \Yii::t('app', Inflector::titleize($key)) . "'>";
                        break;
                    case self::KEY_TYPE_INT:
                        $html .= "<input type='number' " . $required . " value='" . $value . "' class='form-control' name='Value[" . $key . "]' placeholder='" . \Yii::t('app', Inflector::titleize($key)) . "'>";
                        break;
                    case self::KEY_TYPE_EMAIL:
                        $html .= "<input type='email' " . $required . " value='" . $value . "' name='Value[" . $key . "]' class='form-control' placeholder='" . \Yii::t('app', Inflector::titleize($key)) . "'>";
                        break;
                    default:
                        $html .= "<input type='text' " . $required . " value='" . $value . "' name='Value[" . $key . "]' class='form-control' placeholder='" . \Yii::t('app', Inflector::titleize($key)) . "'>";
                        break;
                }
            } else {
                $html .= '<div class="form-group field-gateway-' . $key . '" ' . $required . '>';
            ...
            $html .= '<label class="control-label col-sm-3 col-sm-3" for="gateway-' . $field . '">' . \Yii::t('app', Inflector::titleize($field)) . '</label><div class="col-sm-6">';
            $html .= "<input type='text' class='form-control' name='Value[" . $field . "]' placeholder='" . \Yii::t('app', Inflector::titleize($field)) . "'>";
        }
        $html .= '</div></div>';
        return $html;
    }
