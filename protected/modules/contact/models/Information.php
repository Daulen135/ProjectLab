...
class Information extends \app\components\TActiveRecord
{
    ...
    {
        $model = null;
        $outJson = \Yii::$app->session->get(self::CONTACT_SESSION_KEY);
        if ($outJson) {
        ...
        if ($model == null) {
            ...
            $model->referrer_url = Yii::$app->request->referrer ?: Yii::$app->homeUrl;
            ...
            $model->subject = Yii::$app->request->queryString;
            ...
            if ($id) {
                $port = Portfolio::findOne($id);
                if ($port) {
                    $model->description = 'Need more details on portfolio ' . $port->linkify(null, true);
                }
            }
    ...
    public static function updateNewRecord($model)
    {
        ...
        \Yii::$app->session->set(self::CONTACT_SESSION_KEY, $outJson);
    }

    public function sendToLeadManager($company_id = 1)
    {
        $leadManagerUrl = YII_ENV == 'prod' ? 'http://192.168.10.26/lead/push' : 'http://localhost/lead-manager-web-684/lead/push';

        $json = [];

        $json['title'] = $this->type . ' ==> ' . $this->referrer_url;
        $json['ref_url'] = $this->referrer_url;
        $json['budget'] = $this->budgetType;
        $json['description'] = $this->type . $this->description;
