...
class TAuthHandler
{
        ...
        if (Yii::$app->user->isGuest) {
            if ($auth) { // login
                ...
                if (! ($user->hasErrors())) {
                    ...
                    if (! $auth->save()) {
                         ...
                         * die();
                         */
                        Yii::$app->getSession()->setFlash('error', Yii::t('app', 'Unable to save {client} account: {errors}', [
                    ...
                    Yii::$app->getSession()->setFlash('error', Yii::t('app', 'Unable to save user: {errors}', [
                        'client' => $this->client->getTitle(),
            ...
            if (! $auth) { // add auth provider
                ...
                if ($auth->save()) {
                    ...
                    Yii::$app->getSession()->setFlash('success', Yii::t('app', 'Linked {client} account.', [
                        'client' => $this->client->getTitle()
                    ]));
                } else {
                    Yii::$app->getSession()->setFlash('error', Yii::t('app', 'Unable to link {client} account: {errors}', [
                        'client' => $this->client->getTitle(),
                        'errors' => json_encode($auth->getErrors())
                    ]));
                }
            } else { // there's existing auth
                Yii::$app->getSession()->setFlash('error', Yii::t('app', 'Unable to link {client} account. There is another user using it.', [
                    'client' => $this->client->getTitle()
                ]));
            }
        }
    ...
    public function saveUser($email, $fullName)
    {
        ...
        $attributes = $user->hasAttribute('first_name');

        if ($attributes === true) {
            $exploded = explode(' ', $fullName);
            $user->first_name = $exploded[0];
            $user->last_name = $exploded[1];
        }
